{{-- Monday.com style header with indigo accent for CRM --}}
<header
        ...
        <div class="hidden sm:flex items-center">
            <div class="relative">
                ...
                <input type="text" placeholder="{{ __('Search...') }}"
                    class="w-64 pl-10 pr-4 py-2 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:focus:bg-gray-600 transition">
            </div>
        </div>
    </div>
        ...
        @auth
            @isset($currentTeam)
                <a href="{{ route('dashboard.customers.create', ['team' => $currentTeam]) }}"
                    ...
                    <span class="hidden md:inline">{{ __('New Customer') }}</span>
                </a>
            @endisset
        @endauth
        ...
        @auth
            <div x-data="{ profileOpen: false }" class="relative">
                <button @click="profileOpen = !profileOpen"
                    ...
                    <div
                        class="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-semibold text-sm">
