        ...
        <a href="{{ route('dashboard.customers', ['team' => $currentTeam]) }}" class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($customersCount) }}</p>
                </div>
                <div class="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg flex items-center justify-center group-hover:bg-indigo-200 dark:group-hover:bg-indigo-900/50 transition">
            ...
            <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">
                <span class="text-green-600 dark:text-green-400 font-medium">{{ number_format($activeCustomersCount) }}</span> {{ __('active') }}
            </p>
        </a>
        ...
        <a href="{{ route('dashboard.companies', ['team' => $currentTeam]) }}" class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($companiesCount) }}</p>
                </div>
        ...
        <a href="{{ route('dashboard.contacts', ['team' => $currentTeam]) }}" class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($contactsCount) }}</p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <div class="p-6">
                ...
                @else
                    <p class="text-sm text-gray-500 dark:text-gray-400 text-center py-8">{{ __('No customers yet') }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <div class="p-6">
                ...
                @else
                    <p class="text-sm text-gray-500 dark:text-gray-400 text-center py-8">{{ __('No companies yet') }}</p>
                @endif
            </div>
        </div>
    </div>
